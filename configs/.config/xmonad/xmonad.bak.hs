...
myStartupHook = do
  ...
  spawnOnce "pkill redshift; redshift -P -O 6500; setsid -f redshift &"
  spawnOnce "pkill dunst; setsid -f dunst &"
  spawnOnce "pkill polybar; setsid -f polybar -c ~/.config/polybar/mac.ini &"
  spawnOnce "kill -9 $(pgrep plank); setsid -f plank &"
  spawnOnce "xwallpaper --zoom ~/.config/openbox/assets/wall.jpg &"
  spawnOnce "xset r rate 215 40"
  spawnOnce "xsetroot -cursor_name left_ptr"
  spawnOnce "xrdb -load $HOME/.config/x11/xresources"
  spawnOnce "setxkbmap -option caps:swapescape "
  spawnOnce "setsid -f $HOME/.local/scripts/crondbus"
  spawnOnce " pkill picom; setsid -f picom --config ~/.config/openbox/assets/picom/active.conf & "
...
defaults = def {
      ...
      -- hooks, layouts
        ...
        layoutHook = gaps [(U,50), (D,100), (R,100), (L, 100)] $
